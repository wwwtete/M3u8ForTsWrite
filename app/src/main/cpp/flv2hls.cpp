...
struct VideoFrameData {
    TSWriter::H264FrameType ftype;
    uint32_t ts;
    uint8_t *data;
    int dataLength;
};
std::deque<VideoFrameData> _videoFrames;
...
std::deque<AudioFrameData> _audioFrames;

static void FlushToTSWriter()
{
    static int tsidx = 0;
    
    FILE *idxfile = fopen("/Users/dy/Work/tswriter/tswriter.m3u8", "wb");
    const char idxheader[] = "#EXTM3U\n#EXT-X-VERSION:3\n#EXT-X-MEDIA-SEQUENCE:0\n#EXT-X-ALLOW-CACHE:YES\n#EXT-X-TARGETDURATION:12\n";
    fwrite(idxheader, 1, sizeof(idxheader) - 1, idxfile);
    
    TSWriter tsWriter;
    TSFileBuffer filebuffer;
    tsWriter.AddH264Data(_videoFrames[0].data, _videoFrames[0].dataLength, 
                         _videoFrames[0].ftype, _videoFrames[0].ts, filebuffer);
    tsWriter.AddH264Data(_videoFrames[1].data, _videoFrames[1].dataLength, 
                         _videoFrames[1].ftype, _videoFrames[1].ts, filebuffer);
    _videoFrames.pop_front();
    _videoFrames.pop_front();
    
    while (!_videoFrames.empty() && !_audioFrames.empty()) {
        
        int64_t videoTs = _videoFrames[0].ts;
        int64_t audioTs = _audioFrames[0].ts;
        if (videoTs <= audioTs) {
            tsWriter.AddH264Data(_videoFrames[0].data, _videoFrames[0].dataLength, 
                                 _videoFrames[0].ftype, _videoFrames[0].ts, filebuffer);
            if (filebuffer.data != nullptr) {
                char tmpname[256];
                sprintf(tmpname, "/Users/dy/Work/tswriter/tswriter_%d.ts", tsidx);
                FILE *file = fopen(tmpname, "wb");
                if (file != nullptr) {
                    fwrite(filebuffer.data, 1, filebuffer.ptr, file);
                    fclose(file);
                }
                delete[] filebuffer.data;
                
                char idxitem[256];
                int num = sprintf(idxitem, "#EXTINF:%f,\ntswriter_%d.ts\n", 
                                  (float)filebuffer.duration / 1000.0, tsidx);
                fwrite(idxitem, 1, num, idxfile);
                tsidx++;
            }
            _videoFrames.pop_front();
        }
        else {
            tsWriter.AddAACData(_audioFrames[0].data, _audioFrames[0].dataLength, 0, 0, _audioFrames[0].ts);
            _audioFrames.pop_front();
        }
    }
    
    const char endix[] = "#EXT-X-ENDLIST\n";
    fwrite(endix, 1, sizeof(endix) - 1, idxfile);
    
    tsWriter.Close(filebuffer);
    if (filebuffer.data != nullptr) {
        char tmpname[256];
        sprintf(tmpname, "/Users/dy/Work/tswriter/tswriter_all.ts");
        FILE *file = fopen(tmpname, "wb");
        if (file != nullptr) {
            fwrite(filebuffer.data, 1, filebuffer.ptr, file);
            fclose(file);
...
static hls_frag_t * hls_get_frag(hls_ctx_t  *ctx, int n)
{
    ...
    for (n = 0; ; ++n) {
        for (; nnals; --nnals) {
            ...
            *out->last++ = 1;
            ...
            if (addToVideoFrames == 0) {
                addToVideoFrames = 1;
                VideoFrameData vfdata;
                vfdata.ts = 0;
                vfdata.ftype = TSWriter::H264FrameType::SPS;
                vfdata.data = new uint8_t[4 + len];
                vfdata.dataLength = 4 + len;
                memcpy(vfdata.data, out->last - 4 - len, 4 + len);
                _videoFrames.push_back(vfdata);
            }
            else if (addToVideoFrames == 1) {
                addToVideoFrames = 2;
                VideoFrameData vfdata;
                vfdata.ts = 0;
                vfdata.ftype = TSWriter::H264FrameType::PPS;
                vfdata.data = new uint8_t[4 + len];
                vfdata.dataLength = 4 + len;
                memcpy(vfdata.data, out->last - 4 - len, 4 + len);
                _videoFrames.push_back(vfdata);
...
static void hls_next_frag(hls_ctx_t *ctx)
{
...
static int hls_rename_file(const char *src, const char *dst)
{
...
static int hls_write_playlist(hls_ctx_t *ctx)
{
...
static int hls_close_fragment(hls_ctx_t *ctx, u_int32_t ts)
{
...
static u_int64_t flv_hls_get_fragment_id(hls_ctx_t*ctx)
{
    return ctx->frag + ctx->nfrags;    
}

static int hls_flush_audio(hls_ctx_t *ctx)
{
...
static int hls_open_fragment(hls_ctx_t*ctx, u_int64_t ts, int discont)
{
    u_int64_t                  id;
    int                            fd;
    u_int32_t                  g;
...
static void hls_update_fragment(hls_ctx_t  *ctx, u_int64_t ts, int boundary, u_int32_t flush_rate)
{
    hls_frag_t        *f;
    int                  ts_frag_len;
...
static int hls_parse_aac_header(av_codec_ctx_t   *codec_ctx, u_int32_t *objtype,
...
static int hls_audio(Flv2hlsContext*context, u_int8_t*data, int data_len, u_int32_t timestamp)
{
    ...
    AudioFrameData afdata;
    afdata.ts = timestamp;
    afdata.dataLength = data_len + 5;
    afdata.data = new uint8_t[afdata.dataLength];
    memcpy(afdata.data, p, afdata.dataLength);
    _audioFrames.push_back(afdata);
...
int hls_video(Flv2hlsContext*context, u_int8_t*data, int data_len, u_int32_t timestamp)
{
  ...
  /* 1: keyframe (IDR)
    ...
    while (in - data < data_len) {
        ...
        VideoFrameData vfdata;
        vfdata.ts = timestamp;
        vfdata.ftype = nal_type == 1 ? TSWriter::H264FrameType::P : TSWriter::H264FrameType::I;
        vfdata.dataLength = 4 + len;
        vfdata.data = new uint8_t[len + 10];
        vfdata.data[0] = vfdata.data[1] = vfdata.data[2] = 0;
        vfdata.data[3] = 1;
        vfdata.data[4] = src_nal_type;
        memcpy(vfdata.data + 5, in, len - 1);
        ...
        // doing this after hls_append_sps_pps
        _videoFrames.push_back(vfdata);
...
static void av_codec_parse_aac_header(Flv2hlsContext*context, u_int8_t*data, int data_len)
{
    u_int32_t               idx;
...
static void av_codec_parse_avc_header(Flv2hlsContext*context, u_int8_t*data, int data_len)
{
...
//    char *source = "/Users/dy/Work/livingexample.flv";
//    int c;
//
...
//                hls_video(g_con, (u_int8_t*)data, size, timestamp);
//            }
...
//                hls_audio(g_con, (u_char*)data, size, timestamp);
//            }
//        }
//    }
//    
